...
BASE_URL <- "https://postgrest-seamlessgeolmap-734948684426.us-central1.run.app"

# Cache for storing fetched data
data_cache <- reactiveValues()

# Function to fetch data from PostgREST endpoint with error handling
fetch_postgrest_data <- function(table_name, cache_key = NULL) {
  # Use cache_key if provided, otherwise use table_name
  ...
  endpoint <- paste0(BASE_URL, "/wetdash_", table_name)
  ...
  tryCatch({
    ...
    showModal(modalDialog(
      ...
      paste("Failed to load", table_name, "data from API:", e$message),
...
# Load all required data with error handling and loading indicators
withProgress(message = 'Loading data from API...', value = 0, {
  incProgress(1/10, detail = "Loading site attributes...")
  site_attr <- fetch_postgrest_data('site_attr')
  
  incProgress(1/10, detail = "Loading flags...")
  flags <- fetch_postgrest_data('flags')
  
  incProgress(1/10, detail = "Loading invertebrate data...")
  inverts <- fetch_postgrest_data('inverts')
  
  incProgress(1/10, detail = "Loading invertebrate parameters...")
  invert_taxon <- fetch_postgrest_data('invert_param')
  
  incProgress(1/10, detail = "Loading soil data...")
  soil <- fetch_postgrest_data('soil')
  
  incProgress(1/10, detail = "Loading soil parameters...")
  soil_param <- fetch_postgrest_data('soil_param')
  
  incProgress(1/10, detail = "Loading water data...")
  water <- fetch_postgrest_data('water')
  
  incProgress(1/10, detail = "Loading water parameters...")
  water_param <- fetch_postgrest_data('water_param')
  
  # Optional data - commented out as in original
  # incProgress(1/10, detail = "Loading vegetation data...")
  # veg <- fetch_postgrest_data('veg')
  # veg_param <- fetch_postgrest_data('veg_param')
  
  # incProgress(1/10, detail = "Loading disturbance data...")
  # disturb <- fetch_postgrest_data('disturb')
  # disturb_param <- fetch_postgrest_data('disturb_param')
  
  # incProgress(1/10, detail = "Loading structure data...")
  # structure <- fetch_postgrest_data('structure')
  # structure_param <- fetch_postgrest_data('structure_param')
})

# Check if critical data loaded successfully
if (is.null(site_attr) || is.null(water) || is.null(soil) || is.null(inverts)) {
  showModal(modalDialog(
    title = "Critical Error",
    "Failed to load essential data. Please refresh the page to try again.",
    footer = NULL,
    easyClose = FALSE
  ))
  stop("Critical data failed to load")
}
```

```{r setup for pgadmin, eval=FALSE, include=FALSE}
# This section remains for reference but is not used with API approach
#packages
library(DBI)
...
```{r, reactive refresh function}
# Add a refresh button function that clears cache
observeEvent(input$refresh_data, {
  withProgress(message = 'Refreshing data from API...', value = 0, {
    # Clear cache
    data_cache$site_attr <- NULL
    data_cache$flags <- NULL
    data_cache$inverts <- NULL
    data_cache$invert_param <- NULL
    data_cache$soil <- NULL
    data_cache$soil_param <- NULL
    data_cache$water <- NULL
    data_cache$water_param <- NULL
    
    # Reload data
    incProgress(1/8, detail = "Loading site attributes...")
    site_attr <<- fetch_postgrest_data('site_attr')
    
    incProgress(1/8, detail = "Loading flags...")
    flags <<- fetch_postgrest_data('flags')
    
    incProgress(1/8, detail = "Loading invertebrate data...")
    inverts <<- fetch_postgrest_data('inverts')
    invert_taxon <<- fetch_postgrest_data('invert_param')
    
    incProgress(1/8, detail = "Loading soil data...")
    soil <<- fetch_postgrest_data('soil')
    soil_param <<- fetch_postgrest_data('soil_param')
    
    incProgress(1/8, detail = "Loading water data...")
    water <<- fetch_postgrest_data('water')
    water_param <<- fetch_postgrest_data('water_param')
  })
...
# Remove X column if it exists from API response
...
downloadButton('download_data', 'Download Soil Data')
